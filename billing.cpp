...
void shopping :: menu()
{
    ...
    switch(choice)
    {
        case 1:
            ...
            if(email =="pranjal@gmail.com" && password == "pranjal@123")
            {
                administrator();
            }
            else
...
void shopping :: add()
{
    ...
    if(token == 1) //if there is a duplicasy
    goto m;

    else // if token value is not incremented that means the product code enter by the administrator is unique & not duplicate
    {
        data.open("database.txt", ios::app | ios::out);
        data<<" "<<pcode<<" "<<pname<<" "<<price<<" "<<dis<<"\n";
        data.close();
